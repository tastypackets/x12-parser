...
class X12parser extends Transform {
   ...
   */
  _flush(cb) {
    this._leftOver = this.removeDelimiters(this._leftOver);
  ...
  hasTrailingDelimiter(data) {
    return data[data.length - 1] === this._delimiters.segment;
  }
  ...
  processChunk(chunk) {
    ...
    const addTrailingSegment = this.hasTrailingDelimiter(data);
